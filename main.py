# Import the necessary libraries and establish a connection to the MySQL server
...
def transfer(account_number, receiver_account_number, amount):
    ...
    query = """
        UPDATE accounts SET balance = %s WHERE account_number = %s
    """
    values = [(sender_new_balance, account_number), (receiver_new_balance, receiver_account_number)]
    cursor.executemany(query, values)
    connection.commit()
    print("Transfer successful. Sender's new balance: ${:.2f}".format(sender_new_balance))
...
class Bank:
    ...
    def add_customer(self, first_name, last_name, username, password, initial_balance):
        ...
        self.accounts[account_number] = {'owner': (first_name, last_name), 'balance': initial_balance}
